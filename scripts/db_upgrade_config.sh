...
FILES_TO_DOWNLOAD=$($KPWN_DB_DIR/kpwn_db.py -i kernelctf.kxdb --partial-list-files 2>/dev/null || true)
...
"$KPWN_DB_DIR/kpwn_db.py" -i kernelctf.kxdb --kernel-image-db-path $IMAGE_DB_DIR --partial-sync --output-file kernelctf_new.kxdb
"$KPWN_DB_DIR/kpwn_db.py" -i kernelctf_new.kxdb -o kernelctf_new.json
