...
FILES_TO_DOWNLOAD=$($KXDB_DIR/kpwn_db.py -i kernelctf.kxdb --partial-list-files 2>/dev/null || true)
...
"$KXDB_DIR/kpwn_db.py" -i kernelctf.kxdb --kernel-image-db-path $IMAGE_DB_DIR --partial-sync --output-file kernelctf_new.kxdb
"$KXDB_DIR/kpwn_db.py" -i kernelctf_new.kxdb -o kernelctf_new.json
