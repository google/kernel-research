...
./build.sh
cp build/test/kernelXDKTests ../image_runner/rootfs/test_runner

mkdir -p ../image_runner/rootfs/test/
cp -r test/artifacts ../image_runner/rootfs/test/
...
if [[ "$CUSTOM_MODULES" != "keep" && ( -z "$CUSTOM_MODULES_KEEP" || ! -f "$RELEASE_DIR/custom_modules.tar") ]]; then
    ...
    $SCRIPT_DIR/../image_db/download_release.sh "$DISTRO" "$RELEASE_NAME" "modules"
    $SCRIPT_DIR/../image_runner/compile_custom_modules.sh "$DISTRO" "$RELEASE_NAME" xdk_device
fi
